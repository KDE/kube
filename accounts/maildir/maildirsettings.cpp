...
void MaildirSettings::setAccountIdentifier(const QByteArray &id)
{
    ...
    Sink::Store::fetchOne<Sink::ApplicationDomain::SinkResource>(Sink::Query::PropertyFilter("account", QVariant::fromValue(id)))
...
void MaildirSettings::save()
{
    ...
    qDebug() << "Saving account " << mAccountIdentifier << mIdentifier;
